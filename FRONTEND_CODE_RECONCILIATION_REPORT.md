...
- 探すは正本から算出した処世術数・理論数とカテゴリカードを表示する。
...
- `src/app/welcome.tsx`: 人物像・処世術・理論の表示件数をカタログから導出。
...
既存のGitHub Actions run `33276932259`（commit `c01a162`）が成功し、Pagesへ公開された。1440×900と390×844のChrome新規タブで、Welcome、無料版開始、購買ページ、ホーム、探す、学ぶ、マイページ、カテゴリ、テーマ、人物像一覧、処世術詳細（`master336-001`）、理論詳細（`kb_070`）、学習ケース（`case-01`）、`/legal/privacy` を再確認した。
...
- Chromeの既存タブを使い回した場合に、ブラウザ拡張由来の非同期listener警告が出ることがある。新規タブでのアプリ確認ではReact/hydrationを含むアプリ側console errorは0件だった。
