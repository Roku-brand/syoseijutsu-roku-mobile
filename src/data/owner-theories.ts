...
function toTheory(row: Record<string, unknown>): Omit<TheoryCard, 'status'> & { status: 'published' | 'archived'; displayOrder: number } {
  return { provenance: (row.provenance as TheoryProvenance | null) ?? getTheoryProvenance(bundledTheories.find((item) => item.tagId === row.id) ?? { title: String(row.title ?? '') }), tagId: String(row.id), title: String(row.title ?? ''), summary: String(row.summary ?? ''), categoryId: String(row.category_id ?? 'psychology'), categoryTitle: String(row.category_title ?? ''), aliases: Array.isArray(row.aliases) ? row.aliases as string[] : [], relatedTheoryIds: Array.isArray(row.related_theory_ids) ? row.related_theory_ids as string[] : [], status: row.status === 'archived' ? 'archived' : 'published', displayOrder: Number(row.display_order ?? 0) };
}
