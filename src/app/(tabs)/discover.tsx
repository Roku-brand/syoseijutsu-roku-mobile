...
export default function DiscoverScreen() {
  ...
  return (
    <BookScreen>
      <BookTitle title="探す" subtitle="人物像、処世術、理論から探す。" />
      <View style={styles.searchBox}>
        <AppText style={styles.searchIcon}>⌕</AppText>
        <TextInput value={query} onChangeText={setQuery} placeholder="処世術・人物像・キーワードを探す" placeholderTextColor={colors.muted} accessibilityLabel="処世術を検索" style={styles.searchInput} />
      ...
      {keywords.length ? (
        ...
        <>
          <View accessibilityRole="tablist" style={styles.modeTabs}>
            <Pressable accessibilityRole="tab" accessibilityState={{ selected: mode === 'personas' }} onPress={() => setMode('personas')} style={[styles.modeTab, mode === 'personas' && styles.modeTabActive]}><AppText style={[styles.modeText, mode === 'personas' && styles.modeTextActive]}>人物像から探す</AppText></Pressable>
...
function PersonaBrowser({ router }: { router: ReturnType<typeof useRouter> }) {
  return <View>
    <OrnamentHeading>人物像から探す</OrnamentHeading>
    {categoryOrder.map((key) => {
      ...
      return <View key={key} style={styles.personaSection}>
        ...
        <View style={styles.personaList}>
          {category.subcategories.map((persona, index) => <Pressable key={persona.name} accessibilityRole="button" accessibilityLabel={`${persona.name}、処世術${persona.items.length}件`} onPress={() => router.push({ pathname: '/subcategory/[category]/[name]', params: { category: key, name: persona.name } })} style={({ pressed }) => [styles.personaCard, { borderColor: palette.accent, backgroundColor: palette.tint }, pressed && styles.pressed]}>
            ...
            <View style={styles.personaCopy}><AppText variant="serif" style={styles.personaTitle}>{persona.name}</AppText><AppText style={styles.personaDescription}>{persona.articleTitle}</AppText><AppText variant="label" style={[styles.personaCount, { color: palette.accent }]}>構成する処世術　{persona.items.length}件</AppText></View>
...
const styles = StyleSheet.create({
  ...
  personaSection: { marginTop: spacing.xl }, categoryHeader: { flexDirection: 'row', alignItems: 'center', gap: spacing.md, marginBottom: spacing.md }, categoryMark: { width: 42, height: 42, borderRadius: 21, borderWidth: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surface }, categoryMarkText: { fontFamily: fonts.serif, fontSize: 17, fontWeight: '700' }, categoryCopy: { flex: 1 }, categoryTitle: { fontFamily: fonts.serif, fontSize: 21, lineHeight: 28, fontWeight: '700' }, categoryDescription: { marginTop: 2, color: colors.muted, fontSize: 12, lineHeight: 18 }, categoryCount: { fontFamily: fonts.serif, fontSize: 20, fontWeight: '700' },
  personaList: { gap: spacing.sm }, personaCard: { minHeight: 102, flexDirection: 'row', alignItems: 'center', gap: spacing.md, padding: spacing.md, borderWidth: 1, borderRadius: radius.md }, personaNumber: { width: 44, height: 44, borderRadius: 22, borderWidth: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.surface }, personaNumberText: { fontFamily: fonts.serif, fontSize: 14, fontWeight: '700' }, personaCopy: { flex: 1 }, personaTitle: { fontSize: 18, lineHeight: 26, fontWeight: '700' }, personaDescription: { marginTop: 2, color: colors.inkSoft, fontSize: 12, lineHeight: 18 }, personaCount: { marginTop: 8, fontSize: 11 }, chevron: { fontSize: 30 },
